...
The deploy script snapshots the currently running image before rollout. It waits for both `https://ne-gro.com/version.txt` and `https://www.ne-gro.com/version.txt` to return the exact Git release through Cloudflare before it reports success. Each route must also return `ok` from a non-cacheable `/healthz`, avoid a Cloudflare cache hit for that probe, return a real `404` for a release-scoped missing asset, and provide HSTS, no-store service-worker delivery, CSP framing restrictions, MIME hardening, referrer and permissions policies, and same-origin opener/resource policies. A build-start, container-health, or public-gate failure automatically recreates the service from the snapshotted image and verifies its loopback health and release identity. Git remains on the attempted commit for diagnosis. Set `DARKPIX_PUBLIC_URL` only when intentionally replacing the two-route gate with one alternate hostname.
