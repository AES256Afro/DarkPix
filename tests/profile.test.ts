...
describe("directional combat damage", () => {
  ...
  it("reports monster-triggered traps only within a bounded hearing range", () => {
    expect(trapWarningAudible(0)).toBe(true);
    expect(trapWarningAudible(TRAP_WARNING_RANGE ** 2)).toBe(true);
    expect(trapWarningAudible(TRAP_WARNING_RANGE ** 2 + 0.01)).toBe(false);
    expect(trapWarningAudible(-1)).toBe(false);
    expect(trapWarningAudible(Number.NaN)).toBe(false);
  });
