...
- Save backups and raw recovery evidence briefly attach their download link before cleanup, improving browser consistency without retaining generated object URLs
- A rejected browser download action still removes its temporary link and schedules object-URL revocation through a guaranteed cleanup path
...
- Enemy footstep tracking mutates its stored coordinates in place, compares squared ranges across threats, and computes audible distance only for the chosen cadence source
...
- Pressure plates now sink and glow through a 0.32-second directional warning, then re-evaluate the nearest overlapping body so the delver can sidestep or bait a threat into the release
- A nearby pressure plate depressed by a threat now carries the same directional dodge warning at a quieter pitch, while triggers beyond hearing range remain undisclosed
- Nearby wall ports armed by a threat also signal their incoming volley without exposing distant trap activity
...
- Stash and Contracts header jumps move both viewport and focus into named destinations, with a visible focus target and reduced-motion-aware scrolling
...
- Two-pass production minification preserves the lazy renderer boundary while restoring JavaScript headroom under the unchanged compressed ceiling
...
- Every built JavaScript and CSS hash is captured before replacement, and both rollout and rollback require all public bodies, including lazy game and Three.js chunks, to match the container through both routes
