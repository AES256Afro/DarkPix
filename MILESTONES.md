...
- Offline fallbacks stay inside the active release cache, quoted build imports and CSS asset URLs are walked recursively, cache-write failure preserves valid network responses, incomplete install graphs are rejected and cleaned, and the worker script bypasses runtime caching
...
- Every deploy snapshots the prior image, verifies Cloudflare-uncached live health and release identity, real missing-asset 404s, and the exact commit through both `ne-gro.com` and `www.ne-gro.com`; rollback must prove that both routes serve the restored release before discarding its snapshot
